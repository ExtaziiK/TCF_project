...
- The app is a single-page React app whose route state is mirrored to real URL paths (see `src/constants/seo.js` for the route ↔ path map, e.g. `/tarifs`, `/guide-tcf-canada`). Deep links work (Vite dev falls back to index.html; prod uses vercel.json rewrites); nav/footer links are real `<a href>` anchors driven through `nav()`. `document.title`, canonical and robots meta update per route.
