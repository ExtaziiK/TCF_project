...
// the four are looked up by the `key` in constants/social.js. Filled brand
// glyphs next to lucide's outlined ones is the standard compromise — the marks
// are recognisable shapes, not line drawings.
...
export const SOCIAL_ICON = { youtube: Youtube, facebook: Facebook, tiktok: TikTok, whatsapp: WhatsApp };

// The compact icon row (footer, and anywhere else a discreet follow strip
// fits). Each link is labelled for screen readers — an icon-only link is
// otherwise announced as its URL.
      ...
      {SOCIAL.map((s) => {
        ...
        return (
          <a
            ...
            className={`w-9 h-9 rounded-xl border ${c.border} ${c.sub} ${c.hoverSoft} flex items-center justify-center transition-colors hover:text-blue-600`}
          >
            <Icon size={17} />
          </a>
        );
