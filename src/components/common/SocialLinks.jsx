...
// icons are outlined line-art in the house style — recognisable, but not the
// logos. A follow row is one of the few places where the real mark in the real
// colour is the point: people scan for the shape and the colour together, not
// for a tidy outline.
//
// Each mark carries its own fill, so callers do not colour them. Two need more
// than a flat colour. TikTok's logo is three offset copies of the same glyph —
// cyan and magenta behind, and a body that is black on light and white on dark;
// that last layer is the reason this file reads the theme, and the wider viewBox
// gives the two offsets room so nothing clips at the edges. Instagram's is an
// outline over a gradient, and carries its own <defs>.
...
const TikTok = ({ size = 18 }) => {
  const { dark } = useApp();
  return (
    <svg width={size} height={size} viewBox="-1.5 -1.5 27 27" aria-hidden="true">
      <path d={TIKTOK_PATH} fill="#25F4EE" transform="translate(-1.1 -1.1)" />
      <path d={TIKTOK_PATH} fill="#FE2C55" transform="translate(1.1 1.1)" />
      <path d={TIKTOK_PATH} fill={dark ? "#FFFFFF" : "#010101"} />
    </svg>
  );
};

// Instagram's mark is the one here that is not a flat colour: the glyph is an
// outline filled with the corner-to-corner gradient, so it needs a <defs> of
// its own. useId keeps that def's id unique — the icon renders twice on the
// contact page (footer strip + channel card), and two identical ids in one
// document is invalid markup even where it happens to paint correctly.
const Instagram = ({ size = 18 }) => {
  const id = `ig-${useId()}`;
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill={`url(#${id})`} aria-hidden="true">
      <defs>
...
export const SOCIAL_ICON = { youtube: YouTube, tiktok: TikTok, instagram: Instagram, facebook: Facebook, whatsapp: WhatsApp };
