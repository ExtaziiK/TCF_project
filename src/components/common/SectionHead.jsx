...
// `aside` renders alongside the title on the top-right (stacks below on small
// screens), for a compact widget that belongs next to the heading.
export function SectionHead({ eyebrow, title, sub, center, tight, big, as: Tag = "h2", aside }) {
  const { c } = useApp();
  const head = (
    <div className={`max-w-2xl ${center ? "mx-auto text-center" : ""} ${aside ? "" : tight ? "mb-5" : "mb-10 md:mb-14"}`}>
  ...
  return (
    <div className={`flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6 ${tight ? "mb-5" : "mb-10 md:mb-14"}`}>
      {head}
      <div className="w-full lg:w-auto lg:shrink-0">{aside}</div>
    </div>
  );
}
