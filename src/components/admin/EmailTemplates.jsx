...
import { fetchEmailTemplates, saveEmailTemplate, resetEmailTemplate, sendTestEmail, listPromoCodes } from "@/services/adminService";
import { promoLabel } from "@/services/stripeService";

// Admin › Emails: the copy of the transactional emails the platform sends on
// its own (the renewal reminders). The signup-confirmation and password-reset
// emails are sent by Supabase Auth, not by us, and are edited in the Supabase
// dashboard — the note at the bottom of this tab says so, so nobody hunts for
// them here.
//
// Everything is stored server-side (public.email_templates, admin-only RLS);
// a template that has never been saved renders from the shipped default, which
// is what the editor opens with.

/* ------------------------------- preview ---------------------------------- */

// Mirror of renderTemplate() in api/_lib/emailTemplates.js, for the live
// preview only — what actually goes out is always rendered by the server, so
// this stays a display concern. Sections first, then placeholders.
const HTML_VARS = new Set(["renew_button", "feedback_button"]);
const escapeHtml = (s) =>
  String(s ?? "").replace(/[&<>"]/g, (ch) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;" }[ch]));

function renderPreview(body, vars) {
  const withSections = String(body || "").replace(
    /\{\{#(\w+)\}\}([\s\S]*?)\{\{\/\1\}\}/g,
  ...
  return withSections.replace(/\{\{(\w+)\}\}/g, (_, key) => {
    ...
    return HTML_VARS.has(key) ? String(value) : escapeHtml(value);
  });
}
...
export function EmailTemplatesTab() {
  ...
  const template = useMemo(() => data?.templates.find((t) => t.key === key), [data, key]);

  // Opening a template (or reloading after save/reset) refills the editor.
  useEffect(() => {
    if (template) {
  ...
  // Drops a placeholder where the cursor is, so the admin never has to type the
  // braces (a typo there silently prints the raw tag in a customer's inbox).
  ...
  const previewHtml = useMemo(() => {
    ...
    const vars = { ...data.sample, promo_code: draft.promoCode };
    return data.shell.replace("{{content}}", renderPreview(draft.body, vars));
  }, [data, draft]);

  const previewSubject = useMemo(() => {
    if (!data || !draft) return "";
    return renderPreview(draft.subject, { ...data.sample, promo_code: draft.promoCode });
  }, [data, draft]);
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="p-6">
        <div className="flex items-start justify-between gap-4 flex-wrap mb-5">
          ...
          <div className="flex items-center gap-2 shrink-0">
            {template.customized
              ? <Pill tone="blue">Personnalisé</Pill>
              : <Pill tone="slate">Texte d'origine</Pill>}
        ...
        <div className="space-y-4">
          <div>
            <label className={label} htmlFor="tpl-subject">Objet</label>
            <input id="tpl-subject" value={draft.subject} onChange={(e) => set({ subject: e.target.value })} maxLength={200} className={inp} />
          </div>

          <div>
            <label className={label} htmlFor="tpl-body">Contenu</label>
            <textarea id="tpl-body" ref={bodyRef} value={draft.body} onChange={(e) => set({ body: e.target.value })}
              rows={16} maxLength={8000} spellCheck={false} className={`font-mono2 text-[13px] leading-relaxed ${inp}`} />
            <p className={`text-xs mt-2 ${c.faint}`}>
              L'en-tête et le pied de page Passerelle sont ajoutés automatiquement. Cliquez sur une variable pour l'insérer&nbsp;;
              <code> {"{{#promo_code}}…{{/promo_code}}"}</code> n'apparaît que si un code est choisi.
            </p>
          </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {data.vars.map((v) => (
                <button key={v.name} type="button" onClick={() => insertVar(v.name)} title={v.label}
                  className={`px-2.5 py-1.5 rounded-xl border text-xs font-mono2 ${c.border} ${c.sub} ${c.hoverSoft}`}>
                  {`{{${v.name}}}`}
                </button>
              ))}
            </div>
          ...
          <div>
            ...
            <select id="tpl-promo" value={draft.promoCode} onChange={(e) => set({ promoCode: e.target.value })} className={inp}>
              <option value="">Aucun — le bloc de réduction n'est pas affiché</option>
              {promos.map((p) => (
                <option key={p.id} value={p.code}>{p.code} · {promoLabel(p)}</option>
              ))}
            </select>
            <p className={`text-xs mt-2 ${c.faint}`}>
              Les codes proviennent de l'onglet Promos (codes actifs uniquement). Le même code est envoyé à tous les destinataires.
            </p>
          </div>
        </div>
      ...
      <Card className="p-6">
        ...
        <p className={`text-sm mb-4 ${c.sub}`}>Rendu avec des valeurs d'exemple — objet&nbsp;: <strong className={c.text}>{previewSubject}</strong></p>
        {/* Sandboxed: the preview renders admin-authored HTML, and nothing in an
            email ever needs to run a script. */}
        <iframe title="Aperçu de l'email" srcDoc={previewHtml} sandbox=""
