...
function RegisterGate() {
  ...
  return (
    <PageShell eyebrow="Pratique gratuite" title="Créez un compte gratuit pour commencer" sub="La pratique gratuite est réservée aux membres inscrits. L'inscription prend 30 secondes et ne demande aucune carte bancaire.">
      <Card className="max-w-xl mx-auto p-8">
        <ul className="space-y-3">
          {perks.map((p) => (
            <li key={p} className={`flex items-start gap-3 text-sm ${c.sub}`}>
              <CheckCircle2 size={18} className="text-emerald-500 shrink-0 mt-0.5" />{p}
            </li>
          ))}
        </ul>
        <div className="mt-7 flex flex-col sm:flex-row gap-3">
          <Btn className="flex-1" variant="accent" onClick={() => nav("register")}>Créer un compte gratuitement</Btn>
          <Btn className="flex-1" variant="ghost" onClick={() => nav("login")}>J'ai déjà un compte</Btn>
        </div>
      </Card>
    </PageShell>
  );
...
function UpgradeGate() {
  const { c, nav } = useApp();
  return (
    <PageShell eyebrow="Contenu Premium" title="Ce module fait partie de l'abonnement Premium" sub="Passez au forfait Premium pour débloquer tous les modules, les examens blancs complets et l'analyse IA.">
      <Card className="max-w-xl mx-auto p-8 text-center border-2 border-blue-600/40">
        ...
        <p className={`font-display font-bold text-lg mt-4 ${c.text}`}>Débloquez tout Passerelle</p>
        <p className={`text-sm mt-1.5 ${c.sub}`}>Questions illimitées, les quatre épreuves, examens blancs et suivi CECR dès 19 $ / mois.</p>
        <div className="mt-6 flex justify-center gap-3">
          <Btn variant="accent" onClick={() => nav("pricing")}>Voir les forfaits</Btn>
          <Btn variant="ghost" onClick={() => nav("practice")}>Continuer en gratuit</Btn>
        </div>
      </Card>
    </PageShell>
  );
...
function ForbiddenGate() {
  const { c, nav } = useApp();
  return (
    <PageShell eyebrow="Accès refusé" title="Cette page est réservée à l'administration" sub="Votre compte ne dispose pas des autorisations nécessaires pour consulter cette page.">
      <Card className="max-w-xl mx-auto p-8 text-center">
        <Lock size={28} className={`mx-auto ${c.faint}`} />
        <p className={`text-sm mt-4 ${c.sub}`}>Code d'erreur : 403 — autorisation insuffisante.</p>
        <div className="mt-6 flex justify-center"><Btn variant="ghost" onClick={() => nav("home")}>Retour à l'accueil</Btn></div>
      </Card>
    </PageShell>
  );
}
