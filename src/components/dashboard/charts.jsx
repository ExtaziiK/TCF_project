...
export function ScoreSparkline({ series, height = 88 }) {
  const { c } = useApp();
  if (series.length < 2) {
    return <p className={`text-xs py-6 text-center ${c.faint}`}>Terminez encore quelques quiz pour voir votre courbe de progression.</p>;
  }
  ...
  return (
    <svg viewBox={`0 0 ${w} ${h}`} style={{ height }} className="w-full" role="img" aria-label={`Évolution du score sur les ${series.length} derniers quiz`}>
      {[25, 50, 75].map((g) => (
