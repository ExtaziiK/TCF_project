    ...
    <PageShell back wide eyebrow="Vocabulaire" title="Le lexique de votre nouvelle vie" sub="Cartes mémoire rédigées en contexte canadien. Cliquez une carte pour la retourner ; ajoutez vos mots difficiles aux favoris.">
      <Card className="p-5 mb-8 flex flex-col sm:flex-row items-start sm:items-center gap-4 border-2 border-rose-600/30">
        ...
        <div className="flex-1">
          <p className="text-xs font-bold uppercase tracking-widest text-rose-600">Mot du jour</p>
      ...
      <div className="flex flex-col md:flex-row gap-4 md:items-center justify-between mb-6">
        <div className="flex gap-2 flex-wrap">
          {VOCAB_CATS.map((ct) => (
            <button key={ct} onClick={() => setCat(ct)} className={`px-4 py-1.5 rounded-full text-sm font-semibold transition-colors ${cat === ct ? "bg-blue-600 text-white" : `border ${c.border} ${c.sub} ${c.hoverSoft}`}`}>{ct}</button>
          ))}
        </div>
        <div className="flex gap-2 items-center flex-wrap">
          <div className="relative">
            <Search size={15} className={`absolute left-3.5 top-1/2 -translate-y-1/2 ${c.faint}`} />
            <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Chercher un mot…" aria-label="Chercher un mot" className={`pl-10 pr-4 py-2.5 rounded-full border text-sm outline-none focus:border-blue-600 w-48 ${c.inputCls}`} />
          </div>
          <button onClick={() => setMode(mode === "cards" ? "quiz" : "cards")} className={`px-4 py-2.5 rounded-full text-sm font-semibold flex items-center gap-2 ${mode === "quiz" ? "bg-rose-600 text-white" : `border ${c.border} ${c.sub} ${c.hoverSoft}`}`}>
            <Zap size={14} /> Mode quiz
          </button>
        </div>
      </div>
      {mode === "quiz" ? (
        <Card className="max-w-xl mx-auto p-7 rise">
          <Pill tone="red"><Zap size={12} /> Quiz éclair</Pill>
          <p className={`mt-4 font-medium ${c.text}`}>Quel mot correspond à cette définition : « {quizCard.def} » ?</p>
          ...
          {qSel !== null && <Btn small className="mt-5" icon={RotateCcw} onClick={() => setQSel(null)}>Une autre !</Btn>}
        </Card>
      ) : (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {list.length === 0 && <p className={`col-span-full text-center py-10 text-sm ${c.faint}`}>Aucun mot ne correspond à « {q} » dans cette catégorie.</p>}
          {list.map((v) => {
            ...
            return (
              <div key={v.fr} className="fwrap h-48">
                <div className={`fcard relative w-full h-full ${isFlipped ? "flipped" : ""}`}>
                  <button onClick={() => flip(v.fr)} className={`fface absolute inset-0 rounded-3xl border ${c.border} ${c.card} p-6 flex flex-col items-center justify-center text-center card-lift`} aria-label={`Retourner la carte ${v.fr}`}>
                    <Pill tone="slate" className="absolute top-4 left-4">{v.cat}</Pill>
                    <span onClick={(e) => { e.stopPropagation(); toggleFav(v.fr); }} role="button" aria-label="Favori" className={`absolute top-3.5 right-3.5 p-1.5 rounded-full ${isFav ? "text-rose-600" : c.faint} hover:text-rose-600`}>
                      <Heart size={17} fill={isFav ? "currentColor" : "none"} />
                    </span>
                    <p className={`font-display font-bold text-xl ${c.text}`}>{v.fr}</p>
                    <p className={`mt-2 text-xs ${c.faint}`}>Cliquez pour voir la définition</p>
                  </button>
                  <button onClick={() => flip(v.fr)} className="fface fback absolute inset-0 rounded-3xl p-6 flex flex-col justify-center text-left text-white grad-brand shadow-xl" aria-label="Retourner la carte">
