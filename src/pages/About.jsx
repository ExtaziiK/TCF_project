...
export function About() {
  const { c, t } = useApp();
  return (
    <PageShell back eyebrow={t("À propos")} title={t("Une équipe entre deux continents, un seul objectif : votre réussite")} sub={t("Passerelle est née à Montréal, fondée par des enseignants de FLE et d'anciens candidats au TCF Canada.")}>
      <div className="grid md:grid-cols-2 gap-5 mb-10">
        <Card className="p-7">
          <h3 className={`font-display font-bold text-lg ${c.text} mb-3`}>{t("Notre histoire")}</h3>
          <p className={`text-sm leading-relaxed ${c.sub}`}>{t("En 2022, notre cofondatrice a raté son niveau cible de 8 points, faute de ressources fidèles au format de l'épreuve. Elle a réussi six mois plus tard — puis a bâti la plateforme qu'elle aurait voulu avoir. Aujourd'hui, plus de 12 000 candidats préparent leur test avec Passerelle, depuis 60 pays.")}</p>
        </Card>
        <Card className="p-7">
