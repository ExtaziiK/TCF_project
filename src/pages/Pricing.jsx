    ...
    <PageShell back wide eyebrow="Abonnements" title="Un forfait pour chaque étape de votre préparation" sub="Payez en dollars canadiens, en toute sécurité via Stripe. Changez ou annulez à tout moment depuis votre tableau de bord.">
      ...
      <Card className="mt-10 max-w-xl mx-auto p-6">
        ...
        <div className="flex gap-2">
          <input value={coupon} onChange={(e) => { setCoupon(e.target.value); setApplied(false); }} placeholder="Ex. : BIENVENUE20" aria-label="Code promo" className={`flex-1 px-4 py-3 rounded-2xl border text-sm outline-none focus:border-blue-600 ${c.inputCls}`} />
          <Btn small onClick={() => { if (coupon.trim().toUpperCase() === "BIENVENUE20") { setApplied(true); notify("Code appliqué : −20 % sur votre premier mois !"); } else notify("Ce code n'est pas valide. Essayez BIENVENUE20."); }}>Appliquer</Btn>
        </div>
        {applied && <p className="mt-3 text-sm text-emerald-600 flex items-center gap-1.5 rise"><CheckCircle2 size={15} /> −20 % appliqué sur votre premier mois Premium.</p>}
      </Card>
      <div className={`mt-12 max-w-3xl mx-auto grid sm:grid-cols-3 gap-4 text-center`}>
        {[{ icon: Shield, t: "Paiement chiffré Stripe" }, { icon: RotateCcw, t: "Garantie 30 jours (annuel)" }, { icon: CreditCard, t: "Sans engagement" }].map((b) => (
          <div key={b.t} className={`p-4 rounded-2xl border ${c.border} ${c.card} flex flex-col items-center gap-2`}>
            <b.icon size={20} className="text-blue-600" /><p className={`text-sm font-medium ${c.sub}`}>{b.t}</p>
          </div>
        ))}
      </div>
    </PageShell>
