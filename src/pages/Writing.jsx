  ...
  return (
    <div>
      <div className="flex gap-2 flex-wrap mb-6">
        {tasks.map((t) => (
          <button key={t.task} onClick={() => setActive(t.task)} className={`px-4 py-2 rounded-full text-sm font-semibold transition-colors ${active === t.task ? "bg-blue-600 text-white shadow-lg shadow-blue-600/25" : `border ${c.border} ${c.sub} ${c.hoverSoft}`}`}>
            {t.empty ? `Tâche ${t.task}` : t.t}
          </button>
        ))}
      </div>
        ...
        <Card className="p-6">
          ...
          {showSample ? (
            task.sample
              ...
              : <p className={`mt-3 text-sm ${c.faint}`}>Pas encore d'exemple pour ce sujet.</p>
          ) : (
